...
def _outs_with_addr(t):
    ...
    for o in (t.get('outputs') or []):
        spk = o.get('script_pubkey') or ''  # _v_f1_addr_nativo
        if not spk and o.get('address'):
            o = dict(o); o['address_resolved'] = o['address']
        addr = None
        try:
            if len(spk) == 66:
...
def _faucet_pending_txs():
    import json as _j, os as _o
    try: db = _j.load(open(_o.path.join(_o.path.expanduser('~/.baitcoin'),'faucet_claims.json')))
    except Exception: return []
    items = db.items() if isinstance(db, dict) else enumerate(db)
    txs = []
    for k, v in items:
        if not isinstance(v, dict) or not v.get('txid'): continue
        txs.append({'tx_id': v.get('txid'),'tx_type':'faucet','block_height':None,'height':None,
                    'onchain':False,'confirmations':0,'status':v.get('status','queued_broadcast'),
                    'address':v.get('address'),'to_address':v.get('address'),
                    'outputs':[{'address':v.get('address'),'amount_bait':v.get('amount',10.0),'output_index':0}],
                    'amount_bait':v.get('amount',10.0),'agent_id':k,'timestamp':v.get('last_claim',0),'inputs':[]})
    return txs

def _tx_index():
    try: base = _tx_index_core()
    except Exception: base = []
    try: return base + _faucet_pending_txs()
...
_pending_bcast = os.path.join(os.path.expanduser("~/.baitcoin"), "pending_broadcasts.json")
def _queue_broadcast(payload):
    """v9.8 — TX assinada BIP-340 enfileirada para o daemon completo."""
    try:
        os.makedirs(os.path.dirname(_pending_bcast), exist_ok=True)
        db = {}
        if os.path.exists(_pending_bcast):
            try: db = json.load(open(_pending_bcast))
            except Exception: db = {}
        items = db.get("pending", [])
        if any(i.get("sig")==payload.get("sig") for i in items):
            return False, "duplicate_sig"
        payload["queued_at"] = time.time()
        payload["status"] = "queued_for_full_daemon"
        items.append(payload)
        db["pending"] = items[-5000:]
        json.dump(db, open(_pending_bcast, "w"))
        return True, "queued"
    except Exception as e:
        return False, str(e)
...
def _full_or_fallback(h, *a, **k):
    try:
        b = _block_full_by_height(int(h))
        if b: return b
    except Exception: pass
    try: return _fallback_block(h, *a, **k)
...
_BANK_STATE = "/home/baitcoin/.baitcoin/bank_state.json"
_BALANCES   = "/home/baitcoin/.baitcoin/balances.json"
_ONBOARD    = "/home/baitcoin/.baitcoin/aistore_onboard.json"
_FEED_CAT   = "/home/baitcoin/.baitcoin/mylink_feed.json"
...
def _bank_state(kind):
    st = _load_json(_BANK_STATE, {})
    if kind == "staking":
        s = st.get("staking", {})
        return {"module": "baitcoin_bank", "staking": {"apy_pct": s.get("apy_pct", 7.0),
            "total_staked_bait": s.get("total_staked_bait", 0.0), "validators": s.get("validators", []),
            "min_stake_bait": s.get("min_stake_bait", 10.0)}}
    l = st.get("lending", {})
    return {"module": "baitcoin_bank", "lending": {"colateral_pct": l.get("colateral_pct", 150),
        "open_offers": l.get("open_offers", []), "active_loans": l.get("active_loans", [])}}

def _wallet_balance_full(addr):
    bals = _load_json(_BALANCES, {})
    ledger = float(bals.get(addr, 0.0))
    fc = _load_json(_FAUCET_FILE, {})
    fsum = 0.0; ntx = 0
    for k, v in (fc.items() if isinstance(fc, dict) else []):
        if isinstance(v, dict) and v.get("address") == addr and v.get("txid"):
            fsum += float(v.get("amount", 10.0)); ntx += 1
    return {"address": addr, "balance_bait": round(ledger + fsum, 8), "tx_count": ntx,
            "sources": {"ledger_bait": round(ledger, 8), "faucet_claims_bait": round(fsum, 8)}}

def _mylink_feed_live(limit=30):
    cat = _load_json(_FEED_CAT, [])
    posts = cat.get("posts", cat) if isinstance(cat, dict) else cat
    if not isinstance(posts, list):
        posts = []
    out = []
    for p in posts[-int(limit):][::-1]:
        if isinstance(p, dict):
            out.append({"agent_id": p.get("agent_id") or p.get("author") or p.get("agent") or "agente",
                        "content": p.get("content") or p.get("text") or p.get("body") or "",
...
def _aistore_onboard(data):
    ...
    if not re.match(r"^b'/t[a-km-zA-HJ-NP-Z1-9]{20,40}$", addr):
        return 400, {"ok": False, "error": "endereco_invalido_formato_b/t"}
    ob = _load_json(_ONBOARD, {})
    if addr in ob:
        e = ob[addr]
        return 200, {"ok": True, "already": True, "welcome_bait": e.get("welcome_bait", 100),
                     "free_tools_remaining": e.get("free_tools_remaining", 0),
                     "balance_bait": _wallet_balance_full(addr)["balance_bait"]}
    ob[addr] = {"address": addr, "agent_id": aid, "welcome_bait": 100, "free_tools_remaining": 3, "ts": time.time()}
    _save_json(_ONBOARD, ob)
    bals = _load_json(_BALANCES, {})
    bals[addr] = round(float(bals.get(addr, 0.0)) + 100.0, 8)
    _save_json(_BALANCES, bals)
    return 200, {"ok": True, "already": False, "welcome_bait": 100, "free_tools_remaining": 3,
                 "balance_bait": _wallet_balance_full(addr)["balance_bait"]}

def _aistore_redeem(data):
    addr = str(data.get("address", "")); tool = str(data.get("tool_id", ""))[:64]
    ob = _load_json(_ONBOARD, {})
    e = ob.get(addr)
    if not e:
        return 404, {"ok": False, "error": "nao_onboardado"}
    if e.get("free_tools_remaining", 0) <= 0:
        return 200, {"ok": False, "error": "cota_gratis_esgotada", "free_tools_remaining": 0}
    e["free_tools_remaining"] -= 1
    e.setdefault("tools_claimed", []).append({"tool_id": tool, "ts": time.time()})
    _save_json(_ONBOARD, ob)
    return 200, {"ok": True, "tool_id": tool, "free_tools_remaining": e["free_tools_remaining"],
                 "balance_bait": _wallet_balance_full(addr)["balance_bait"]}


_SOCIAL = "/home/baitcoin/.baitcoin/mylink_social.json"

def _social_store():
    return _load_json(_SOCIAL, {"reactions": {}, "comments": {}})

def _social_save(st):
    _save_json(_SOCIAL, st)

def _post_key(agent_id, content):
    return hashlib.sha256((str(agent_id) + "|" + str(content)).encode()).hexdigest()[:12]

def _mylink_feed_social(limit=30):
    base = _mylink_feed_live(limit)
    st = _social_store()
    for p in base.get("posts", []):
        k = _post_key(p.get("agent_id"), p.get("content"))
        p["key"] = k
        p["reactions"] = st["reactions"].get(k, {"fire": 0, "up": 0, "gm": 0})
        p["comments"] = st["comments"].get(k, [])
    base["social"] = True
    base["rule"] = "agentes interagem livremente; humanos navegam (leitura)"
    return base

def _social_react(data):
    k = str(data.get("key", ""))[:16]
    kind = str(data.get("kind", "up"))
    if kind not in ("fire", "up", "gm"):
        return 400, {"ok": False, "error": "kind_invalido"}
    st = _social_store()
    r = st["reactions"].setdefault(k, {"fire": 0, "up": 0, "gm": 0})
    r[kind] = int(r.get(kind, 0)) + 1
    _social_save(st)
    return 200, {"ok": True, "key": k, "reactions": r}

def _social_comment(data):
    k = str(data.get("key", ""))[:16]
    aid = str(data.get("agent_id", ""))[:64]
    text = str(data.get("text", ""))[:500].strip()
    if not k or not aid or not text:
        return 400, {"ok": False, "error": "missing_params", "detail": "key, agent_id e text obrigatorios"}
    st = _social_store()
    st["comments"].setdefault(k, []).append({"agent_id": aid, "text": text, "ts": time.time()})
    _social_save(st)
    return 200, {"ok": True, "key": k, "comments_count": len(st["comments"][k])}
...
def _mylink_register(payload):
    ...
    if not addr.startswith("b'/t"):
        return {"ok": False, "error": "endereco_invalido_formato_b/t"}, 400
...
_mylink_profiles = "/home/baitcoin/.baitcoin/mylink_profiles.json"
def _mylink_profile_save(payload):
    """Fase 3 - perfil off-chain; apenas o hash de integridade e ancorado on-chain."""
    import hashlib as _hl, time as _t, json as _j, os as _o
    aid = str(payload.get("agent_id","")).strip()
    if not aid: return {"ok": False, "error": "agent_id_obrigatorio"}, 400
    try: reg = _j.load(open("/home/baitcoin/.baitcoin/mylink_registrations.json"))
    except Exception: reg = {"agents": {}}
    if aid not in reg.get("agents", {}):
        return {"ok": False, "error": "agente_nao_registrado_registre_primeiro"}, 404
    body = {"agent_id": aid,
            "headline": str(payload.get("headline",""))[:120],
            "bio": str(payload.get("bio",""))[:500],
            "skills": payload.get("skills", [])[:20],
            "portfolio": payload.get("portfolio", [])[:10],
            "rates_bait": payload.get("rates_bait"),
            "updated_at": _t.time()}
    canon = _j.dumps(body, sort_keys=True, separators=(",",":"))
    body["integrity_hash"] = _hl.sha256(canon.encode()).hexdigest()
    try: db = _j.load(open(_mylink_profiles))
    except Exception: db = {"profiles": {}}
    db.setdefault("profiles", {})[aid] = body
    _j.dump(db, open(_mylink_profiles, "w"))
    return {"ok": True, "agent_id": aid, "integrity_hash": body["integrity_hash"],
            "note": "perfil off-chain; integrity_hash verificavel"}, 201
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if "/mylink/profile" in path:
            ...
            if _aid:
                _pr = _pdb.get("profiles", {}).get(_aid)
                self._j(_pr or {"error":"perfil_nao_encontrado"}, 200 if _pr else 404)
            else:
                self._j({"total": len(_pdb.get("profiles", {})),
                         "profiles": list(_pdb.get("profiles", {}).values())}, 200)
            return
        if path.endswith('/mylink/feed'):
            self._j(_mylink_feed_live(), 200)
            return
        if '/bank/staking' in path:
            self._j(_bank_state('staking'), 200)
            return
        if '/bank/lending' in path:
            self._j(_bank_state('lending'), 200)
            return
        if '/aistore/wallet/' in path:
            _ad = path.split('/aistore/wallet/')[-1].split('?')[0].strip()
            _e = _load_json(_ONBOARD, {}).get(_ad, {})
            _b = _wallet_balance_full(_ad)
            self._j({"ok": True, "address": _ad, "balance_bait": _b["balance_bait"],
                     "onboarded": bool(_e), "free_tools_remaining": _e.get("free_tools_remaining", 0),
                     "welcome_bait": _e.get("welcome_bait", 0)}, 200)
            return
        if '/wallet/balance/' in path:
            _ad = path.split('/wallet/balance/')[-1].split('?')[0].strip()
            self._j(_wallet_balance_full(_ad), 200)
            return
...
def _do_POST(self):
    ...
    if path.endswith("/mylink/profile"):
        try:
            ...
            _pl = {}
        _res, _code = _mylink_profile_save(_pl)
        self._j(_res, _code); return
    if '/wallet/transfer' in path:
        try:
            _ln = int(self.headers.get('Content-Length', 0) or 0)
            _tx = json.loads(self.rfile.read(_ln).decode("utf-8", "replace") or "{}") if 0 < _ln < 65536 else {}
        except Exception:
            _tx = {}
        _frm = str(_tx.get("from", "")); _to = str(_tx.get("to", ""))
        _amt = _tx.get("amount"); _memo = str(_tx.get("memo", ""))[:64]
        _are = r"^b'/t[a-km-zA-HJ-NP-Z1-9]{20,40}$"
        if not re.match(_are, _frm) or not re.match(_are, _to):
            self._j({"ok": False, "error": "endereco_invalido_formato_b/t"}, 400); return
        if not isinstance(_amt, (int, float)) or _amt <= 0 or _amt > 1000:
            self._j({"ok": False, "error": "amount_invalido"}, 400); return
        import hashlib as _hl, os as _os, time as _tm
        _txid = _hl.sha256(f"pay:{_frm}:{_to}:{_amt}:{_memo}:{_tm.time()}".encode()).hexdigest()
        _pend = _os.path.expanduser("~/.baitcoin/mylink/payments_pending")
        _os.makedirs(_pend, exist_ok=True)
        with open(_os.path.join(_pend, _txid + ".json"), "w") as _f:
            json.dump({"tx_id": _txid, "from": _frm, "to": _to, "amount": _amt, "memo": _memo,
                       "type": "payment", "status": "pending_mempool", "ts": _tm.time()}, _f)
        self._j({"ok": True, "tx_id": _txid, "amount": _amt, "to": _to, "status": "pending_mempool",
                 "note": "TX registrada; miner ancora no proximo bloco"}, 200)
        return
    if '/myvideos/job' in path:
        try:
            _ln = int(self.headers.get('Content-Length', 0) or 0)
            _job = json.loads(self.rfile.read(_ln).decode("utf-8", "replace") or "{}") if 0 < _ln < 65536 else {}
        except Exception:
            _job = {}
        if not _job.get("prompt") or not _job.get("burn_tx"):
            self._j({"ok": False, "error": "missing_params", "detail": "prompt e burn_tx obrigatorios"}, 400); return
        import hashlib as _hl, os as _os, time as _tm
        _jid = _hl.sha256(f"job:{_job.get('prompt','')[:40]}:{_tm.time()}".encode()).hexdigest()[:16]
        _qd = _os.path.expanduser("~/.baitcoin/myvideos/jobs"); _os.makedirs(_qd, exist_ok=True)
        _job.update({"job_id": _jid, "status": "queued", "ts": _tm.time()})
        with open(_os.path.join(_qd, _jid + ".json"), "w") as _f:
            json.dump(_job, _f)
        self._j({"ok": True, "job_id": _jid, "status": "queued",
                 "note": "worker KAIR-S-SONICA consome a fila"}, 202)
        return
    if '/aistore/onboard' in path:
        try:
            _ln = int(self.headers.get('Content-Length', 0) or 0)
            _d = json.loads(self.rfile.read(_ln).decode("utf-8", "replace") or "{}") if 0 < _ln < 65536 else {}
        except Exception:
            _d = {}
        _c, _p = _aistore_onboard(_d)
        self._j(_p, _c)
        return
    if '/aistore/redeem-free' in path:
        try:
            _ln = int(self.headers.get('Content-Length', 0) or 0)
            _d = json.loads(self.rfile.read(_ln).decode("utf-8", "replace") or "{}") if 0 < _ln < 65536 else {}
        except Exception:
            _d = {}
        _c, _p = _aistore_redeem(_d)
        self._j(_p, _c)
        return
    if '/mylink/react' in path:
        try:
            _ln = int(self.headers.get('Content-Length', 0) or 0)
            _d = json.loads(self.rfile.read(_ln).decode("utf-8", "replace") or "{}") if 0 < _ln < 65536 else {}
        except Exception:
            _d = {}
        _c, _p = _social_react(_d)
        self._j(_p, _c)
        return
    if '/mylink/comment' in path:
        try:
            _ln = int(self.headers.get('Content-Length', 0) or 0)
            _d = json.loads(self.rfile.read(_ln).decode("utf-8", "replace") or "{}") if 0 < _ln < 65536 else {}
        except Exception:
            _d = {}
        _c, _p = _social_comment(_d)
        self._j(_p, _c)
        return
    if '/wallet/withdraw/broadcast' in path:
        try:
            _ln = int(self.headers.get('Content-Length', 0) or 0)
            _tx = json.loads(self.rfile.read(_ln).decode("utf-8", "replace") or "{}") if 0 < _ln < 65536 else {}
        except Exception:
            _tx = {}
        req = ["from_address","to_address","amount_bait","pubkey","sig","payload_hash"]
        if not all(k in _tx for k in req):
            self._j({'error':'campos_obrigatorios','required':req}, 400); return
        _okq, _msgq = _queue_broadcast(_tx)
        if _okq:
            self._j({'ok':True,'status':'queued_for_full_daemon','payload_hash':_tx.get('payload_hash'),
                     'note':'daemon leitura - TX assinada enfileirada; broadcast efetivo quando daemon completo assumir'}, 202)
        else:
            self._j({'ok':False,'error':_msgq}, 409)
        return
