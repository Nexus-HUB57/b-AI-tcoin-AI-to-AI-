...
if [ "$c" = 200 ] && grep -q '"chain_valid": true' /tmp/v01.json && grep -qE '"chain_height": ?[0-9]{3,}' /tmp/v01.json; then
  PASS=$((PASS+1)); row C01 PASS "status 200 chain_valid=true $(grep -oE '"chain_height": ?[0-9]+' /tmp/v01.json)"
else
...
if [ "$c" = 200 ] && grep -q '"persistent": true' /tmp/v02.json && grep -qE '"height": ?[0-9]{3,}' /tmp/v02.json; then
...
if [ "$c" = 200 ] && grep -qE '"BTC": ?[1-9]' /tmp/v03.json && grep -qE '"ETH": ?[1-9]' /tmp/v03.json; then
...
if [ "$c" = 200 ] && grep -q '"hash"' /tmp/v06.json; then
...
# ---- C08: frontend /blockchain (HTML sem "undefined" — via API já coberto) --
c=$(code "$BASE/blockchain" /tmp/v08.txt)
if [ "$c" = 200 ] && grep -qi '<html' /tmp/v08.txt; then
...
c=$(curl -s -L -m 20 -o /tmp/v09.txt -w '%{http_code}' "$BASE/aistore/" 2>/dev/null || echo 000)
if [ "$c" = 200 ] && grep -qi '<html' /tmp/v09.txt; then
  PASS=$((PASS+1)); row C09 PASS "/aistore/ 200 sem loop (nginx B2 ok)"
else
  FAIL=$((FAIL+1)); row C09 FAIL "/aistore/ -> $c (loop nginx — B2 pendente)"
fi

# ---- C10: aistore/api/stats ------------------------------------------
c=$(code "$BASE/aistore/api/stats" /tmp/v10.json)
if [ "$c" = 200 ] && grep -q '"total"' /tmp/v10.json; then
...
if [ "$c" = 200 ] && ! grep -qi 'Page cannot be displayed' /tmp/v11.txt && grep -q '"' /tmp/v11.txt; then
