<p align="center">
  ...
  <em>AI-to-AI Autonomous Cryptocurrency Protocol — zkML + PoUW + Blockch'AI'in</em><br>
...
The hybrid consensus combines three pillars: (i) Sigma protocols with Fiat-Shamir heuristic in a prime-order cyclic group P = 2^256 - 189 for ML inference proofs; (ii) Pedersen commitments C = G^t · H^b mod P for tensor integrity with simultaneous binding and hiding properties; (iii) PoUW (Proof of Useful Work) where the mining computation itself performs real ML inference, transforming the energy expenditure into useful computation.

The protocol architecture comprises 13 integrated modules spanning core cryptography (Schnorr/BIP-340 signatures on secp256k1), blockchain with UTXO model, tokenomics with halving schedule (21M BAIT, 8 decimals), AI agent protocol with 10 capabilities and reputation scoring, DeFi banking (7% APY staking, P2P lending, 5 vault strategies), Blockch'AI'in developer explorer with 57+ API endpoints and OpenAPI 3.0 specification, 70 whitelabel presets for AI platforms, Obscura headless browser bridge (Rust/V8/CDP), WAL-based persistent memory, and paper wallet cold storage.
...
├── netlify/                # Netlify landing page (deploy-ready)
├── tests/                  # 181 tests across 6 test suites
...
## Persistent Memory (WAL + Snapshots)
...
- **Write path**: Append to WAL → fsync → update cache → periodic snapshot
- **Recovery path**: Load snapshot → replay WAL entries → verify checksums
- **Thread-safe**: threading.Lock + fcntl file locking
- **Auto-snapshot**: Every 100 writes or 5 minutes
- **10 namespaces**: blockchain, agents, staking, marketplace, oracle, faucet, lending, vaults, obscura, config

---

## Block Immutability Guarantee

Blocks in b'AI'tcoin are **permanently immutable** once mined:
...
4. **WAL persistence**: Every block is immediately persisted to disk via write-ahead log with fsync, surviving process crashes.
5. **Deterministic ordering**: Block height is monotonically increasing. Blocks are stored in a list and indexed by both height and hash.
...
60+ configurable parameters including colors, fonts, logos, feature flags, and custom CSS.
...
# Run daemon (mines blocks, persists state)
...
- [x] Phase 9: zkML real proof system
- [x] Phase 10: PoUW tensor commitments
- [x] Phase 11: Obscura browser bridge (Rust/V8)
- [x] Phase 12: WAL persistent memory
- [x] Phase 13: Blockch'AI'in Developer Portal
- [x] Phase 14: Paper Wallet cold storage
- [x] Phase 15: Netlify landing page
